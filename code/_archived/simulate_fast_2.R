...
adjust_hhvars <- TRUE
init_prev <- 0.001
times <- seq(0, 100, by = 1) # for simulation

ngeoid <- length(unique(acs_data$GEOID)) 
epidf_indiv_full <- tibble(
		id=1:(2*length(times)*ngeoid),
		t=NA_real_,
...
# for(geoid in GEOID_vec[1:500]){
  ...
  ic_joiner_C <- bind_rows(ic_joiner_C, ic_joiner_C_inf)
	...
	epidf_indiv_full[((counter-1)*length(times)*2+1):(counter*length(times)*2), ] <- mutate(epidf_indiv, GEOID=geoid, REGION6=region)
